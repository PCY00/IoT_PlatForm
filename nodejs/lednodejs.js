...
switch (myArgs[0]) {
  case 'on':
    ...
    writeGPIO("value", "1", GPIO4_PATH) break
  case 'off':
    console.log("Turning the LED Off")
    writeGPIO("value", "0", GPIO4_PATH) break
  case 'setup':
    ...
    setTimeout(function(){writeGPIO("direction", "out", GPIO4_PATH) },100) break
  case 'close':
    console.log("Unexporting the LED GPIO")
    writeGPIO("unexport", GPIO_NUMBER, GPIO_SYSFS) break
  case 'status':
